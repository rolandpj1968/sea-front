...
#define GTY(x)
#define gcc_assert(x)
#define gcc_checking_assert(x)
#define gcc_unreachable() abort()
#define ATTRIBUTE_UNUSED
#define hashval_t unsigned int

// Stub the deps hash-table.h needs
template<typename T> void *xcalloc(size_t n, size_t s) { return malloc(n*s); }
inline void *xmalloc(size_t s) { return malloc(s); }
...
struct test_descriptor {
    typedef int value_type;
    static hashval_t hash(int v) { return (hashval_t)v; }
    static bool equal(int a, int b) { return a == b; }
    static void remove(int) {}
    static void mark_deleted(int &v) { v = -1; }
    static void mark_empty(int &v) { v = 0; }
